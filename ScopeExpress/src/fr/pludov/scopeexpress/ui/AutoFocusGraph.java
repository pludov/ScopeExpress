...
import fr.pludov.scopeexpress.ui.ShapeGraphPanel.StarOccurenceView;
import fr.pludov.scopeexpress.utils.WeakListenerOwner;

public class AutoFocusGraph extends JPanel {
	...
	private AutoFocusTask.Interpolation interpolation;
	...
	void loadTaskStatus(AutoFocusTask runningTask, int passId)
	{
		...
		for(int stepId = 0; stepId < runningTask.parameters.getStepCount(passId); ++stepId)
		{
			int pos = runningTask.parameters.getPassPos(runningTask.getPassCenter(passId), passId, stepId);
	...
	class Scaler
	{
		...
		{
			return xMargin + (step - minPos) * xScale;
		}

		public double pix2step(int x) {
			// (step - minPos) * xScale + xMargin= x;
			return (x - xMargin) / xScale + minPos;
		}
