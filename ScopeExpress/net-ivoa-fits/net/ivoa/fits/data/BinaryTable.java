...
public class BinaryTable extends Data implements TableData {
	...
	public BinaryTable() throws FitsException {
		try {
			...
			System.err
					.println("Impossible exception in BinaryTable() constructor"
							+ e);
		}
		heap = new FitsHeap(0);
		extendArrays(0);
	...
	public BinaryTable(Header myHeader) throws FitsException {
		...
		if (heapSize > 0) {
			heapOffset = myHeader.getIntValue("THEAP", nRow * rwsz);
		}
	 ...
	 * 
	 * If the TDIMS value cannot be deciphered a one-d array with the size given
	 * in arrsiz is returned.
	 * 
	 * @param tdims
	...
	public void fillHeader(Header h) throws FitsException, HeaderCardException {
	...
	public void pointToColumn(int col, Header hdr) throws FitsException {
	...
	private ColumnTable createTable() throws FitsException {
		...
		for (int i = 0; i < nfields; i += 1) {
			arrCol[i] = ArrayFuncs.newInstance(ArrayFuncs
					.getBaseClass(modelRow[i]), sizes[i] * nRow);
		}
	...
	private Object[] getFileRow(int row) throws FitsException {
		...
		for (int col = 0; col < data.length; col += 1) {
			data[col] = ArrayFuncs.newInstance(ArrayFuncs
					.getBaseClass(modelRow[col]), sizes[col]);
		}
	 ...
	 * 
	 * @param o
	 *            An array of objects instantiating the data. These should have
	 *            the same structure as any existing rows.
	 */
	 ...
	 * and the second dimension is not constant.
	 */
	...
	private Object columnToArray(int col, Object o) throws FitsException {
		...
		if ((flags[col] & COL_VARYING) != 0) {
			...
			if ((flags[col] & COL_STRING) != 0) {
				int[] dims = dimens[col];
				o = FitsUtil.byteArrayToStrings((byte[]) o,
						dims[dims.length - 1]);
