...
public class HeaderCard {
	 ...
	 * The keyword for the card will be "HIERARCH.LEV1.LEV2..." The value will
	 * be the first token which starts with a non-alphabetic character (i.e.,
	 * not A-Z or _). A '/' is assumed to start a comment.
	 */
		...
		if (value != null || nullable) {
			...
			if (value != null) {
				...
				if (avail > 0 && b.length() < 29) {
					b.append(space80.substring(0, Math.min(avail, 29 - b
							.length())));
				}
				...
				if (isString) {
					...
					b.append(space80.substring(0, Math.min(avail, 10 - value
							.length())));
				}
				b.append(value);
				if (isString) {
