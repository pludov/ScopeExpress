...
public class ArrayFuncs implements PrimitiveInfo {
	...
	public static Object deepClone(Object o) {
		...
		if (classname.charAt(ndim) != 'L') {
			...
			try {
				...
				System.err
						.println("Internal error: class definition inconsistency: "
								+ classname);
				return null;
			}
		}
	 ...
	 * reflection to invoke the clone method. This can't be done directly since
	 * as far as the compiler is concerned the clone method for Object is
	 * protected and someone could implement Cloneable but leave the clone
	 * method protected. The cloning can fail in a variety of ways which are
	 * trapped so that it returns null instead. This method will generally
	 * create a shallow clone. If you wish a deep copy of an array the method
	 * deepClone should be used.
	 * 
	 * @param o
	 ...
	 * 
	 * This method returns an integer array with the dimensions of the object o
	 * which should usually be an array.
	 * 
	...
	protected static int doCurl(Object input, Object output, int[] dimens,
		...
		for (int i = 0; i < dimens[0]; i += 1) {
			total += doCurl(input, ((Object[]) output)[i], xdimens, offset
					+ total);
		}
		return total;
	}
