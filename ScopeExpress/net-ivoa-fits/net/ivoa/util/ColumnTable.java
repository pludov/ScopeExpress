 ...
 * read and written using the BufferedDataXputStream classes.
 * 
 * The table is represented entirely as a set of one-dimensional primitive
 * arrays. For a given column, a row consists of some number of contiguous
 * elements of the array. Each column is required to have the same number of
 * rows.
 */
...
public class ColumnTable implements DataTable {
	...
	public void addRow(Object[] row) throws TableException {
		...
		if (arrays.length == 0) {
			...
			for (int i = 0; i < row.length; i += 1) {
				...
				Object xarray = ArrayFuncs.newInstance(bases[i], (nrow + 1)
						* sizes[i]);
	 ...
	 *            implementations may use other methods to store the data (e.g.,
	...
	private int checkColumnConsistency(Object data, String classname,
			int ratio, int size) throws TableException {
