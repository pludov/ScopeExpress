...
public class PolynomialFitter {
	...
	public static class Polynomial extends ArrayList<Double> {
		...
		public double dichoFindZero(double a, double b)
		{
			double siga = Math.signum(getY(a));
			while(true) {
				double nva = (a + b) / 2;
				if (nva == a || nva == b) return nva;
				double nvsig = Math.signum(getY(nva));
		...
		List<Double> solve(double from, double to)
		{
			if (size() == 1) {
				...
				double b = get(1);
				// a + x * b = 0
				List<Double> resultList = new ArrayList<Double>(1);
				double result = -a / b;
				...
				// Deriver.
				Polynomial derivate = derivate();

				// Trouver où la derivee s'annule
				List<Double> points = derivate.solve(from, to);
				
				// Maintenant trouver les passages à 0 par dichotomie
				points.add(from);
				points.add(to);
				...
				for(int i = 0; i < points.size() - 1; ++i)
				{
					...
					double siga = Math.signum(getY(a));
					double sigb = Math.signum(getY(b));
					if (siga == 0) {
						if (i == 0) {
							result.add(a);
						}
		...
		public double findMin(double from, double to)
		{
			List<Double> possibles = derivate().solve(from, to);
			possibles.add(from);
			possibles.add(to);
			Double bestY = null;
			Double result = null;
			for(Double r : possibles)
			{
				Double y = getY(r);
				if (bestY == null || bestY > y) {
					bestY = y;
					result = r;
				}
			}
