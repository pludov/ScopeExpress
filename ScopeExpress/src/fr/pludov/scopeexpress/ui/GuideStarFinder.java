...
import fr.pludov.scopeexpress.ui.settings.InputOutputHandler.BooleanConverter;
import fr.pludov.scopeexpress.ui.settings.InputOutputHandler.DegConverter;
import fr.pludov.scopeexpress.ui.settings.InputOutputHandler.DoubleConverter;
		...
		Utils.addComboChangeListener(this.setupSelector, new Runnable() {
			...
			public void run() {
				...
				try {
					...
					new fr.pludov.scopeexpress.utils.EndUserException("Erreur de lecture", e).report(GuideStarFinder.this); 
				}
				refreshSetupBtonStatus();
			}
		});
		...
		this.btnSetupAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (newName != null) {
					...
					if (existing.exists()) {
						new fr.pludov.scopeexpress.utils.EndUserException("Existe déjà").report(GuideStarFinder.this);
					} else {
						...
						try {
							...
							new fr.pludov.scopeexpress.utils.EndUserException("Impossible de sauvegarder", ex).report(GuideStarFinder.this);
						}
					}
				}
			}
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (ssi.location.exists()) {
					if (ssi.location.delete()) {
						...
						new fr.pludov.scopeexpress.utils.EndUserException("Impossible de supprimer").report(GuideStarFinder.this);
					}
				}
			}
		});
